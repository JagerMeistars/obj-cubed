/**
 * objmc BlockBench Plugin v2.0.0
 * Port of https://github.com/Godlander/objmc
 *
 * Features:
 *   - Custom "objmc Model" project type (mesh + bone rig + animation + display transforms)
 *   - Animation baking: BB keyframe animations → objmc frames
 *   - Per-context models: @gui, @head, @ground, @fixed groups → separate PNGs, one combined JSON
 *   - Full display transform output (reads from BB Display tab if available)
 */
(function () {
    'use strict';

    const PLUGIN_ID = 'objmc';
    ...
    BBPlugin.register(PLUGIN_ID, {
        ...
        description: 'Export the current model with objmc encoding for Minecraft resource packs',
        icon: 'icon',
        version: '2.5.2',
        min_version: '4.8.0',
        variant: 'desktop',
        onload() {
            this.exportAction = new Action(PLUGIN_ID + '_export', {
                name: 'Export as objmc…',
                description: 'Encode current model + texture into objmc format',
                icon: 'icon',
                click: showDialog,
            });
    ...
    async function compileAnimatedObjFrames(anim, opts) {
        ...
        try {
            ...
            for (let i = 0; i < nframes; i++) {
                ...
                const c = filterArm ? await withArmatureHidden(compile) : compile();
    ...
    function buildMaterialToTexMap(mtlMap) {
        ...
        for (const [mtlName, texFile] of mtlMap) {
            ...
            const baseName = texFile.replace(/^.*[/\\]/, '').replace(/\.\w+$/, '');
            const idx = Texture.all.findIndex(t => {
                const tName = (t.name || '').replace(/\.\w+$/, '');
                return tName === baseName || tName === texFile;
            });
    ...
    async function buildOutput(cfg, objContents, mtlString) {
        ...
        try {
            ...
            if (decNv !== nvertices) console.error(`[objmc-verify] nvertices: encoded=${decNv} expected=${nvertices}`);
            if (decNf !== nframes)   console.error(`[objmc-verify] nframes: encoded=${decNf} expected=${nframes}`);
            ...
            if (posDiff > 0.01) console.error(`[objmc-verify] pos[0] MISMATCH: exp=[${expPos.map(v=>v.toFixed(4))}] dec=[${decPos.map(v=>v.toFixed(4))}]`);
            ...
            if (decPi !== srcVert[0]) console.error(`[objmc-verify] vert[0].pos: encoded=${decPi} expected=${srcVert[0]}`);
            if (decUi !== srcVert[1]) console.error(`[objmc-verify] vert[0].uv: encoded=${decUi} expected=${srcVert[1]}`);
            ...
            if (nframes > 1) {
                ...
                if (srcLast) {
                    if (decLPi !== srcLast[0]) console.error(`[objmc-verify] vert[${lastIdx}].pos: enc=${decLPi} exp=${srcLast[0]}`);
                }
            }
        } catch(e) { console.error('[objmc-verify] error:', e.message); }

        const pngBuffer = encodePNG(tw, ty, buf);
        const p0 = [buf[0],buf[1],buf[2],buf[3]];
        const markerOk = p0[0]===12&&p0[1]===34&&p0[2]===56&&p0[3]===78;
        const debugInfo = `${nfaces} faces · ${nframes} frame(s) · ${tw}×${ty}px · marker(${p0.join(',')}) ${markerOk?'✓':'✗'}`;
    ...
    async function getObjContents(cfg, onProgress) {
        ...
        try {
            if (cfg.animationEnabled) {
                ...
                const compile = () => Codecs.obj.compile();
                const c = cfg.filterArmature ? await withArmatureHidden(compile) : compile();
                ...
                methods: {
                    browseDatapackDir() {
                        ElecDialogs.showOpenDialog({
                            ...
                            defaultPath: this.datapackOutputDir || undefined,
                        }).then(result => {
                            if (!result.canceled && result.filePaths[0]) {
                                this.datapackOutputDir = result.filePaths[0];
                            }
                        });
                    },
                    onAnimChange() {
...
<div style="padding:14px 16px;font-size:13px;line-height:1.6;">
      ...
      <div v-if="showDatapackOption" style="margin-top:6px;padding:6px 8px;background:rgba(255,255,255,0.03);border-radius:3px;">
        ...
        <div v-if="generateDatapack" style="margin-top:6px;font-size:12px;">
          ...
          <div style="color:#666;margin-top:4px;font-size:11px;line-height:1.5;">
            Init (once): <span style="color:#aaa;font-family:monospace;">function {{datapackNamespace}}:animations/{{datapackAnimId}}/init</span><br/>
            Play: <span style="color:#aaa;font-family:monospace;">execute as @e[...] run function {{datapackNamespace}}:animations/{{datapackAnimId}}/play</span><br/>
            Stop: <span style="color:#aaa;font-family:monospace;">execute as @e[...] run function {{datapackNamespace}}:animations/{{datapackAnimId}}/stop</span><br/>
            Tick (play_once only): <span style="color:#aaa;font-family:monospace;">function {{datapackNamespace}}:animations/{{datapackAnimId}}/tick</span><br/>
            Commands: play, stop, set, play_from, play_once
          </div>
        </div>
      </div>
    </div>
    ...
    <div v-show="showDisplay" style="padding:0 10px 10px 10px;font-size:12px;">
      <div style="margin-bottom:8px;">
        ...
        <div style="display:grid;grid-template-columns:auto 1fr 1fr 1fr;gap:4px;align-items:center;">
          ...
          <input v-model="dThirdRX" type="number" step="1" style="width:100%;" placeholder="X"/>
          <input v-model="dThirdRY" type="number" step="1" style="width:100%;" placeholder="Y"/>
          <input v-model="dThirdRZ" type="number" step="1" style="width:100%;" placeholder="Z"/>
          <span style="color:#888;font-size:11px;">Pos</span>
          <input v-model="dThirdTX" type="number" step="0.5" style="width:100%;" placeholder="X"/>
          <input v-model="dThirdTY" type="number" step="0.5" style="width:100%;" placeholder="Y"/>
          <input v-model="dThirdTZ" type="number" step="0.5" style="width:100%;" placeholder="Z"/>
        </div>
      </div>
      <div style="margin-bottom:8px;">
        ...
        <div style="display:grid;grid-template-columns:auto 1fr 1fr 1fr;gap:4px;align-items:center;">
          <span style="color:#888;font-size:11px;">Rot</span>
          <input v-model="dHeadRX" type="number" step="1" style="width:100%;"/>
          <input v-model="dHeadRY" type="number" step="1" style="width:100%;"/>
          <input v-model="dHeadRZ" type="number" step="1" style="width:100%;"/>
          <span style="color:#888;font-size:11px;">Pos</span>
          <input v-model="dHeadTX" type="number" step="0.5" style="width:100%;"/>
          <input v-model="dHeadTY" type="number" step="0.5" style="width:100%;"/>
          <input v-model="dHeadTZ" type="number" step="0.5" style="width:100%;"/>
        </div>
      </div>
      <div style="margin-bottom:8px;">
        ...
        <div style="display:grid;grid-template-columns:auto 1fr 1fr 1fr;gap:4px;align-items:center;">
          <span style="color:#888;font-size:11px;">Rot</span>
          <input v-model="dGroundRX" type="number" step="1" style="width:100%;"/>
          <input v-model="dGroundRY" type="number" step="1" style="width:100%;"/>
          <input v-model="dGroundRZ" type="number" step="1" style="width:100%;"/>
          <span style="color:#888;font-size:11px;">Pos</span>
          <input v-model="dGroundTX" type="number" step="0.5" style="width:100%;"/>
          <input v-model="dGroundTY" type="number" step="0.5" style="width:100%;"/>
          <input v-model="dGroundTZ" type="number" step="0.5" style="width:100%;"/>
        </div>
      </div>
      <div>
        ...
        <div style="display:grid;grid-template-columns:auto 1fr 1fr 1fr;gap:4px;align-items:center;">
          <span style="color:#888;font-size:11px;">Rot</span>
          <input v-model="dFixedRX" type="number" step="1" style="width:100%;"/>
          <input v-model="dFixedRY" type="number" step="1" style="width:100%;"/>
          <input v-model="dFixedRZ" type="number" step="1" style="width:100%;"/>
          <span style="color:#888;font-size:11px;">Pos</span>
          <input v-model="dFixedTX" type="number" step="0.5" style="width:100%;"/>
          <input v-model="dFixedTY" type="number" step="0.5" style="width:100%;"/>
          <input v-model="dFixedTZ" type="number" step="0.5" style="width:100%;"/>
        </div>
      </div>
    </div>
  </div>
  ...
  <div style="display:flex;gap:10px;align-items:center;">
    <button @click="doExport" :disabled="running" style="padding:6px 24px;font-size:14px;">
